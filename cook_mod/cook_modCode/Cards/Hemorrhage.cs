    ...
    CardRarity.Rare, TargetType.AnyEnemy)
{
    ...
    protected override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay cardPlay)
    {
        if (!cardPlay.Target.HasPower<BleedPower>())
            return;
        await PowerCmd.Apply<BleedPower>(choiceContext, cardPlay.Target, cardPlay.Target.GetPowerAmount<BleedPower>() * this.DynamicVars["Mult"].IntValue, this.Owner.Creature, this);
    }
    
    protected override void OnUpgrade()
    {
        this.DynamicVars["Mult"].UpgradeValueBy(1m);
    }
}